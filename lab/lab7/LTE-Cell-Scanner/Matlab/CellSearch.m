...
function [cell_info, r_pbch_sub, r_20M_sub, cell_info_return] = CellSearch(r_pbch, r_20M, f_search_set, ...
          fc, sampling_carrier_twist, pss_peak_max_reserve, num_pss_period_try, combined_pss_peak_range, ...
          par_th, num_peak_th)
...
for try_idx = 1 : num_try
    ...
    [period_ppm, dynamic_f_search_set, xc, ~, ~, ~, ~, extra_info] = sampling_ppm_f_search_set_by_pss(capbuf_pbch.', ...
        f_search_set, td_pss, pss_fo_set, sampling_carrier_twist, pss_peak_max_reserve, num_pss_period_try, ...
        combined_pss_peak_range, par_th, num_peak_th);
    
    %figure(3); 
    %show_time_frequency_grid_according_pss(extra_info.pss_loc, extra_info.k_factor, r_pbch_sub);
    if sampling_carrier_twist==0
        ...
        for i=1:length(k_factor_set)
            ...
            tmp_peak=peak_search(xc_incoherent_collapsed_pow,xc_incoherent_collapsed_frq,Z_th1,dynamic_f_search_set(i),...
                                 fc,sampling_carrier_twist,k_factor_set(i));
