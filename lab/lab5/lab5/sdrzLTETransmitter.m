%% LTE Transmitter with Analog Devices FMCOMMS1 RevB/C
...
% <matlab:showdemo('sdrzLTEReceiver') LTE Receiver with Analog Devices(TM)
% FMCOMMS1 RevB/C> example if you have a second SDR platform.
%
...
% looping transmission of these eight frames with the Zynq radio hardware
...
% using the companion example <matlab:showdemo('sdrzLTEReceiver') LTE
% Receiver with Analog Devices FMCOMMS1 RevB/C>, if you have a second SDR
% platform.
...
% work in conjunction with the <matlab:showdemo('sdrzLTEReceiver') LTE
% Receiver with Analog Devices FMCOMMS1> example.

% Check that LTE System Toolbox is installed, and that there is a valid license
if isempty(ver('lte')) % Check for LST install
    error('sdrzLTETransmitter:NoLST','Please install LTE System Toolbox to run this example.');
elseif ~license('test', 'LTE_Toolbox') % Check that a valid license is present
    error('sdrzLTETransmitter:NoLST','A valid license for LTE System Toolbox is required to run this example.');
end

%% Running the Example
% The example can be run by executing <matlab:edit('sdrzLTETransmitter.m')
% sdrzLTETransmitter.m>. The transmitter is controlled using the values in
...
% system object is used with the named radio |'ZC706 and FMCOMMS1 RevB/C'|
% to transmit baseband data to the SDR hardware. The SDR hardware is
% configured to use an intermediate frequency of 30 MHz by default.
%
...
% RevB/C'| with |'ZedBoard and FMCOMMS1 RevB/C'| in the |txsim| parameter
% structure to run with ZedBoard(TM) and ADI FMCOMMS1 hardware.

txsim.RadioDACRate = 122.88e6;
txsim.RadioIntermediateFrequency = 30e6;
txsim.RadioCenterFrequency = txsim.DesiredCenterFrequency-txsim.RadioIntermediateFrequency;
txsim.RadioInterpolation = txsim.RadioDACRate/txsim.SamplingRate;
txsim.SDRDeviceName = 'ZC706 and FMCOMMS1 RevB/C';
...
hSDR = sdrtx( ...
    ...
    'DACRate',               txsim.RadioDACRate, ...
    'InterpolationFactor',   txsim.RadioInterpolation, ...
    'IntermediateFrequency', txsim.RadioIntermediateFrequency, ...
...
% Reshape the baseband LTE data into frames
...
%% Things to Try
% The companion example <matlab:showdemo('sdrzLTEReceiver') LTE Receiver
% with SDR Hardware> can be used to decode the broadcast channel of the
