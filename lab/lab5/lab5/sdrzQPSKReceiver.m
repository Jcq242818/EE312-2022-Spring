...
% sent by the <matlab:showdemo('sdrzQPSKTransmitter') QPSK Transmitter
% with Analog Devices(TM) FMCOMMS1> example. The receiver demodulates the
% received symbols and prints a simple message to the MATLAB command line.
%
...
%
% In this example, the <matlab:sdrzdoc('commsdrrxzc706fmc1')
...
% signal sources:
%
% * The <matlab:showdemo('sdrzQPSKTransmitter') QPSK Transmitter with
% Analog Devices FMCOMMS1> MATLAB example.
% * The <matlab:showdemo('sdrzqpsktx') QPSK Transmitter with Analog Devices
% FMCOMMS1> Simulink(R) example.
...
%% Running the Example
%
% The example can be run by executing <matlab:edit('sdrzQPSKReceiver.m')
% sdrzQPSKReceiver.m>. Make sure you have a suitable transmitter running
% before starting the receiver.
%
% The receiver initialization script, 
% <matlab:edit('sdrzqpskreceiver_init.m') sdrzqpskreceiver_init.m>,
% initializes the simulation parameters and generates the structure
% _prmQPSKReceiver_.

prmQPSKReceiver = sdrzqpskreceiver_init % Receiver parameter structure
%%
% Make sure that the _prmQPSKReceiver.DesiredRFCenterFrequency_ variable
...
% centered at 2.415 GHz. This is achieved using a radio center frequency of
% 2.412 GHz and an intermediate frequency of 3 MHz. The expected signal
% center frequency can be changed by setting the
% _SimParams.DesiredRFCenterFrequency_ variable in the
% <matlab:edit('sdrzqpskreceiver_init.m') receiver initialization file>.
%
...
%%
% By default, the example is configured to run with ZC706 and ADI FMCOMMS1 hardware. 
...
% % prmQPSKReceiver.SDRDeviceName='ZedBoard and FMCOMMS1 RevB/C';

prmQPSKReceiver.SDRDeviceName = 'ZC706 and FMCOMMS1 RevB/C';
...
if compileIt
    fprintf('Compiling runSDRZQPSKReceiver...');
    codegen('runSDRZQPSKReceiver', '-args', {coder.Constant(prmQPSKReceiver)});
    fprintf('done!\n');
end
if useCodegen
    fprintf('Running using compiled code\n');
    fprintf('\n==== Starting Reception ====\n');
    clear runSDRZQPSKReceiver_mex
    [BER,lostSamples] = runSDRZQPSKReceiver_mex(prmQPSKReceiver);
else
    fprintf('Running using uncompiled code\n');
    fprintf('\n==== Starting Reception ====\n');
    [BER,lostSamples] = runSDRZQPSKReceiver(prmQPSKReceiver);
end
...
%%
% The function <matlab:edit('runSDRZQPSKReceiver.m') runSDRZQPSKReceiver>
% implements the QPSK receiver using two system objects:
% <matlab:edit('sdrzQPSKRx.m') sdrzQPSKRx> and
% <matlab:sdrzdoc('commsdrrxzc706fmc1')
% SDR Receiver>. For a Simulink block diagram of the
% system, refer to the <matlab:showdemo('sdrzqpskrx') QPSK Receiver with
...
% the *commsdrrxzc706fmc1* system object to use the default
...
% _CenterFrequency_, _Gain_, and _DecimationFactor_ arguments.
%
% *QPSK Receiver*
%
...
% more information, see Chapter 7 of [ <#14 1> ]. The PLL uses a *Direct
...
% [ <#14 1> ], to correct the timing error in the received signal. The *NCO
% Control* object implements a decrementing modulo-1 counter described in
% Chapter 8.4.3 of [ <#14 1> ] to generate the control signal for the
...
% alpha set to 0.5 as described in Chapter 8.4.2 of [ <#14 1> ]. Based on
% the interpolants, timing errors are generated by a zero-crossing *Timing
% Error Detector* as described in Chapter 8.4.1 of [ <#14 1> ]. They are
% then filtered by a tunable proportional-plus-integral *Loop Filter* as
% described in Appendix C.2 of [ <#14 1> ], and fed into the *NCO Control*
...
% SDR Hardware. You can also view a Simulink implementation of this example
% in <matlab:showdemo('sdrzqpskrx') QPSK Receiver with Analog Devices
% FMCOMMS1 using Simulink>.
%
...
% *commsdrrxzc706fmc1* system object by changing the _SimParams.RadioGain_
% value in the in the <matlab:edit('sdrzqpskreceiver_init.m') receiver
% initialization file>.
...
%
% * <matlab:edit('runSDRZQPSKReceiver.m') runSDRZQPSKReceiver.m>: a codegen
