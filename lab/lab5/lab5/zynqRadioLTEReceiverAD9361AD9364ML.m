...
% companion <matlab:showdemo('zynqRadioLTETransmitterAD9361AD9364ML') LTE Transmitter with 
...
% <matlab:showdemo('zynqRadioLTETransmitterAD9361AD9364ML') LTE Transmitter with 
% Analog Devices AD9361/AD9364> example.
...
% structure. The sample rate of the receiver is 1.92 MHz, which is the
% standard sample rate for capturing an LTE bandwidth of 6 resource blocks
% (RBs). 6 RBs is equivalent to a signal bandwidth of 1.4 MHz. You may wish
% to increase the _rx.numBurstCaptures_ parameter to capture more bursts of
% frames. Five LTE frames are captured in each burst.
...
% FDD duplexing mode and a normal cyclic prefix length are assumed. Four
...
while rxsim.numBurstCaptures
    ...
    for frame = 1:rxsim.FramesPerBurst
        while len == 0
            ...
            [data,len,lostSamples] = step(hSDR);
...
release(hSDR); % Release SDR receiver object
release(hsa);  % Release spectrum analyzer object
release(hcd);  % Release constellation diagram object

%% Things to Try
% The companion example <matlab:showdemo('zynqRadioLTETransmitterAD9361AD9364ML') LTE
% Transmitter Using Analog Devices AD9361/AD9364> can be used to transmit a
% standard compliant LTE waveform which can be partially decoded by this
% example. In the companion example try changing the cell identity and
% initial frame number and observe the detected cell identity and frame
