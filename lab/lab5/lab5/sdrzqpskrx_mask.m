function varargout = sdrzqpskrx_mask(block, action, varargin)
% sdrzqpskrx_mask  Sets up workspace variables for the SDR QPSK Rx example
...
if isnan(FFTSize) || ~isreal(FFTSize) || mod(log2(FFTSize),1)~=0 || FFTSize<=0
    error(message('SDR:qpskrxmask:PositiveFFTSize'));
end
% Assign parameter to the block
CoarseFreqCompblock = [bdroot '/QPSK Receiver' ...
        '/Coarse Frequency Compensation/Find Frequency Offset'];
myException = sdrzUpdateTgtParamFromEdit(block, CoarseFreqCompblock, 'fftLen', 'FFTSize');
...
if isnan(numSpecAverage) || ~isreal(numSpecAverage) || mod(numSpecAverage,1)~=0 || numSpecAverage<=0
    error(message('SDR:qpskrxmask:PositiveNumeSpecAverage'));
end
% Assign parameter to the block
CoarseFreqCompblock = [bdroot '/QPSK Receiver' ...
        '/Coarse Frequency Compensation/Find Frequency Offset'];
myException = sdrzUpdateTgtParamFromEdit(block, CoarseFreqCompblock, 'fftAvgs', 'numSpecAverage');
...
myException = sdrzUpdateTgtParamFromEdit(block, FineFreqCompblock, 'BnTs', 'FineFreqNormalizedLoopBW');
...
FineFreqCompblock = [bdroot '/QPSK Receiver' ...
        '/Fine Frequency Compensation/Loop Filter'];
myException = sdrzUpdateTgtParamFromEdit(block, FineFreqCompblock, 'zeta', 'FineFreqDampingFactor');
...
myException = sdrzUpdateTgtParamFromEdit(block, TimeRecLoopFilterblock, 'BnTs', 'TimeRecNormalizedLoopBW');
...
TimeRecLoopFilterblock = [bdroot '/QPSK Receiver' ...
        '/Timing Recovery/Timing Recovery PLL/Loop Filter'];
myException = sdrzUpdateTgtParamFromEdit(block, TimeRecLoopFilterblock, 'zeta', 'TimeRecDampingFactor');
