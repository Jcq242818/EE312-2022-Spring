function sdrzWalkieTalkieRxHelper_ModelParamsMask(action)
%SDRZWALKIETALKIERXHELPER_MODELPARAMSMASK Mask control for walkie-talkie example
%
% This function, and its associated 'Model Parameters' block in the
% sdrzWalkieTalkieRx example, is used to:
...
%     sdrzWalkieTalkieRxHelper_ModelParamsMask('applyParametersToModel');
%
...
%     sdrzWalkieTalkieRxHelper_ModelParamsMask('maskDisplayUpdate');
%
...
CTCSSBlock = find_system(bdroot, 'Name', 'CTCSSCode');
set_param(CTCSSBlock{1}, 'value', maskParameters.CTCSSCode);

squelchThresholdBlock = find_system(bdroot, 'Name', 'Squelch Threshold');
set_param(squelchThresholdBlock{1}, ...
    'value', maskParameters.signalPowerThreshold);

decodeBlockSizeBlock = find_system(bdroot, 'Name', 'decodeBlockSize');
set_param(decodeBlockSizeBlock{1}, ...
    'value', maskParameters.CTCSSDecodeBlockLength);
...
maskParameters.signalPowerThreshold = ...
    get_param(modelParametersBlockPath, 'signalPowerThreshold');
maskParameters.CTCSSDecodeBlockLength = ...
    get_param(modelParametersBlockPath, 'CTCSSDecodeBlockLength');
end
...
validateattributes( ...
    str2double(maskParameters.CTCSSDecodeBlockLength), ...
    {'numeric'}, ...
    {'scalar', 'real', '>', 0});
validateattributes( ...
...
validateattributes( ...
    ...
    {'scalar', 'real', '>=', 1, '<=', 8});
validateattributes( ...
    str2double(maskParameters.signalPowerThreshold), ...
    {'numeric'}, ...
    {'scalar', 'real', '>=', 0});
validateattributes( ...
    str2double(maskParameters.CTCSSDecodeBlockLength), ...
